...
namespace Connector::V12::Thread {
    ...
    const MMAI::Schema::V12::ISupplementaryData* Connector::extractSupplementaryData(const MMAI::Schema::IState *s) {
        ...
        auto &t = typeid(const MMAI::Schema::V12::ISupplementaryData*);
        auto err = MMAI::Schema::AnyCastError(any, typeid(const MMAI::Schema::V12::ISupplementaryData*));

        if(!err.empty()) {
            LOGFMT("anycast for getSumpplementaryData error: %s", err);
        }

        return std::any_cast<const MMAI::Schema::V12::ISupplementaryData*>(s->getSupplementaryData());
    };
    ...
    const P_State Connector::convertState(const MMAI::Schema::IState* s) {
        ...
        assert(sup->getType() == MMAI::Schema::V12::ISupplementaryData::Type::REGULAR);
        ...
        constexpr ssize_t dstate = MMAI::Schema::V12::BATTLEFIELD_STATE_SIZE;
        constexpr ssize_t dmask = MMAI::Schema::V12::N_ACTIONS;
    ...
    const std::tuple<int, std::string> Connector::render(int side) {
        ...
        assert(sup->getType() == MMAI::Schema::V12::ISupplementaryData::Type::ANSI_RENDER);
    ...
    void Connector::start() {
        ...
        if (red == "MMAI_RANDOM") {
            ...
            leftModel = new ML::ModelWrappers::Function(version(), "MMAI_MODEL", Side::LEFT, f_getAction0, f_getValueDummy);
        ...
        if (blue == "MMAI_RANDOM") {
            ...
            rightModel = new ML::ModelWrappers::Function(version(), "MMAI_MODEL", Side::RIGHT, f_getAction1, f_getValueDummy);
