...
class VcmiEnv(gym.Env):
    ...
    ACTION_SPACE = gym.spaces.Dict({
        ...
        "action_2": gym.spaces.Discrete(N_AMOVE_ACTIONS)
    })

    OBSERVATION_SPACE = gym.spaces.Dict({
        "observation": gym.spaces.Box(low=STATE_VALUE_NA, high=1, shape=(STATE_SIZE,), dtype=np.float32),
        "action_mask_1": gym.spaces.Box(low=0, high=1, shape=(N_PRIMARY_ACTIONS,), dtype=bool),
        "action_mask_2": gym.spaces.Box(low=0, high=1, shape=(N_PRIMARY_ACTIONS, 165), dtype=bool)
    })
    ...
    def step(self, action_dict, fallback=False):
        a1 = action_dict["action_1"]
        a2 = action_dict["action_2"]

        # print(".", end="", flush=True)
        if (self.terminated or self.truncated) and a1 != -1:
        ...
        if a1 == 0 and not self.allow_retreat:
            if self.allow_invalid_actions:
                ...
                self.logger.error("Attempted a retreat action %s, but retreat is not allowed" % action_dict)

        action_int = (a2 << 8) | a1
        res = self.connector.step(action_int)
    ...
    def _build_dict_obs(self, res):
        return {
            "observation": res.state,
            "action_mask_1": res.actmask[:N_PRIMARY_ACTIONS],
            "action_mask_2": np.vstack((
                np.zeros((N_PRIMARY_ACTIONS - N_AMOVE_ACTIONS, 165), dtype=bool),
