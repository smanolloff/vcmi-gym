...
if (os.getenv("VCMIGYM_DEBUG", None) == "1"):
    from ...connectors.build import exporter_v5
else:
    from ...connectors.rel import exporter_v5

EXPORTER = exporter_v5.Exporter()
...
N_PRIMARY_ACTIONS = len(PRIMARY_ACTION_MAP.keys())
N_AMOVE_ACTIONS = len(AMOVE_ACTION_MAP.keys())
...
class PyProcConnector():
    ...
    def _get_connector(self):
        if (os.getenv("VCMIGYM_DEBUG", None) == "1"):
            ...
            from ...connectors.build import connector_v5
        else:
            from ...connectors.rel import connector_v5
