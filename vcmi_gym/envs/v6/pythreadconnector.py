...
if (os.getenv("VCMIGYM_DEBUG", None) == "1"):
    from ...connectors.build import exporter_v6
else:
    from ...connectors.rel import exporter_v6

EXPORTER = exporter_v6.Exporter()
...
class PyThreadConnector():
    ...
    def start_connector(self, *args):
        ...
        if (os.getenv("VCMIGYM_DEBUG", None) == "1"):
            ...
            from ...connectors.build import connector_v6
        else:
            from ...connectors.rel import connector_v6

        self.logger.debug("VCMI connector args: %s" % str(args))
        self._connector = connector_v6.ThreadConnector(
            self.maxlogs,
            self.boot_timeout,
            self.vcmi_timeout,
            self.user_timeout,
