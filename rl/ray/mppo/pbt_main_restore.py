...
from .util import deepmerge, to_abspath, vcmigym_root_path, data_path, silence_log_noise
...
def build_master_config(cfg):
    ...
    git = pygit2.Repository(vcmigym_root_path)
...
if __name__ == "__main__":
    ...
    # import logging
    # logging.getLogger('ray.tune.schedulers.pbt').setLevel(logging.DEBUG)
    # logging.getLogger('ray.tune.execution.tune_controller').setLevel(logging.DEBUG)

    # XXX: Tuner.restore() still does not work (with PBT at least) as of 2.38.0
    if "experiment" in pbt_config["load_options"]:
        restore_path = to_abspath(pbt_config["load_options"]["experiment"]["path"])
        ...
        print("*** STARTING NEW EXPERIMENT %s ***" % pbt_config["experiment_name"])
