...
from ..constants_v10 import GLOBAL_ATTR_MAP
from ..util.timer import Timer


class S3DatasetMask(IterableDataset):
    def __init__(
        self,
        logger,
        bucket_name,
    ...
    def _build_filelist(self):
        ...
        # Available types in each sample: ["obs", "mask", "done", "action", "reward"]
        # NOTE: "action" is the action taken while in "obs" (not before it)
        ...
        with self.timer_all:
            while True:
                ...
                for prefix in worker_prefixes:
                    ...
                    for obs, mask, done in zip(samples["obs"], samples["mask"], samples["done"]):
                        index_battle_in_progress = GLOBAL_ATTR_MAP["BATTLE_WINNER"][1] == 1  # 1 if winner is N/A (explicit null => index=offset)

                        # battle-in-progress means non-zero mask
                        assert obs[index_battle_in_progress] == done
                        assert any(mask) == done
