...
from .pb2_config import config  # noqa: E402


def main():
    assert len(sys.argv) == 3, "usage: python rl.raytune.pb2 mppo <name>"
    alg = sys.argv[1]
    desc = sys.argv[2]
    assert alg in ["mppo"], "Only mppo is supported for PB2"
    assert re.match(r"^[0-9A-Za-z_-].+$", desc)
    experiment_name = "PB2-%s-%s" % (desc, datetime.now().strftime("%Y%m%d_%H%M%S"))
